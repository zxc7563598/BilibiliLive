...
final class CreateBiliQuestionUsers extends AbstractMigration
{
    /**
     * Migrate Up.
     */
    public function up()
    {
        $table = $this->table('bili_question_users', ['id' => false, 'primary_key' => 'user_id', 'comment' => '投稿用户表']);
        $table->addColumn('user_id', 'uuid', ['comment' => '用户id', 'null' => false])
            ->addColumn('account', 'string', ['comment' => '账号', 'null' => true])
            ->addColumn('real_name', 'string', ['comment' => '昵称', 'null' => true])
            ->addColumn('email', 'string', ['comment' => '邮件', 'null' => true])
    ...
    {
        $this->table('bili_question_users')->drop()->save();
    }
}
