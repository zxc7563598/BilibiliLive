 ...
 */
class BoxAuthCheck implements MiddlewareInterface
{
    public function process(Request $request, callable $next): Response
    {
        ...
        Redis::hIncrBy(config('app')['app_name'] . ':box:request', $path, 1);
        $param = $request->all();
        // 验证时间是否正确
        ...
        Redis::hIncrBy(config('app')['app_name'] . ':box:success', $path, 1);
        // 处理成功，返回阶段
        if (isset($request->res['code']) && $request->res['code'] == 0) {
            Redis::hIncrBy(config('app')['app_name'] . ':box:error', $path, 1);
        }
        return $response;
    }
}
